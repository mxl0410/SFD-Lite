...
class BboxLoss(nn.Module):
    """Canonical box loss with an explicit, recorded regression mode.

    ``SFD_CANONICAL_REGRESSION=ciou`` selects the official CIoU regression
    branch. ``dynwd`` selects the frozen Dynamic CIoU-NWD branch. Assignment
    always uses the restored official ``metrics.bbox_iou`` implementation.
    """
    ...
    def __init__(
        ...
        nwd_constant: float = 8.0,
    ):
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.nwd_weight = nwd_weight
        self.nwd_constant = nwd_constant
        self.regression_mode = os.environ.get("SFD_CANONICAL_REGRESSION", "").strip().lower()
        if self.regression_mode not in {"ciou", "dynwd"}:
            raise RuntimeError(
                "SFD_CANONICAL_REGRESSION must be explicitly set to 'ciou' or 'dynwd'; "
                f"received {self.regression_mode!r}"
            )

    @staticmethod
    def _official_ciou_loss(box1, box2):
        """Return the official 1-CIoU regression loss for aligned xyxy boxes."""
        return 1.0 - bbox_iou(box1, box2, xywh=False, CIoU=True).squeeze(-1)

    def _dynamic_ciou_nwd_loss(self, box1, box2, eps=1e-7):
        """
        Quality-aware Dynamic CIoU-NWD Fusion Loss.
        Returns per-box loss (shape: [N]).
        """
        ...
        # CIoU geometry is computed locally to keep the DyNWD formula explicit.
        ...
        wasserstein = (
            rho2
            + ((w1 - w2).pow(2) + (h1 - h2).pow(2)) / 4.0
        )
        nwd = torch.exp(
            -torch.sqrt(wasserstein + eps)
            / self.nwd_constant
        )

        quality = ((ciou.detach() + 1.0) / 2.0).clamp(0.0, 1.0)
        weight_nwd = self.nwd_weight * (1.0 - quality).pow(2)

        # ---------- Loss ----------
        loss_ciou = 1.0 - ciou
        loss_nwd = 1.0 - nwd
        loss = (1.0 - weight_nwd) * loss_ciou + weight_nwd * loss_nwd
        return loss
    ...
    def forward(
        ...
        selected_pred = pred_bboxes[fg_mask]
        selected_target = target_bboxes[fg_mask]
        loss_iou_value = (
            self._official_ciou_loss(selected_pred, selected_target)
            if self.regression_mode == "ciou"
            else self._dynamic_ciou_nwd_loss(selected_pred, selected_target)
        )

        loss_iou = (
            loss_iou_value.unsqueeze(-1) * weight
