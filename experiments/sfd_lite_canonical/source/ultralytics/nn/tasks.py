...
def parse_model(d, ch, verbose=True):
    ...
    detect_legacy = d.get("detect_legacy")
    ...
    base_modules = frozenset(
        {
            ...
            SPPELAN, C2fAttn, C3, C3TR, C3Ghost, torch.nn.ConvTranspose2d,
            DWConvTranspose2d, C3x, RepC3, PSA, SCDown, C2fCIB, A2C2f,
            CA_SPD, RepC3k2, SA_SPD, SF_DCA, SPDConv,
            SF_DCA_Lite,
            Stride2ConvBaseline, SPDOriginalPrototype, PixelUnshuffleConv1x1, PixelUnshuffleConv1x1DW, SA_SPD_Lite,
        }
    )
    repeat_modules = frozenset(  # modules with 'repeat' arguments
        {
            BottleneckCSP, C1, C2, C2f, C3k2, C2fAttn, C3, C3TR, C3Ghost,
            C3x, RepC3, C2fPSA, C2fCIB, C2PSA, A2C2f, RepC3k2,
        })
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in frozenset({AFDA, DySample, LargeKernelAttention}):
            c2 = ch[f]
            args = [ch[f], *args]
        elif m is P2_VisDrone_Block:
            c1 = ch[f]
            c2 = args[1] if len(args) > 1 else args[0]
            ...
            args.extend([reg_max, end2end, [ch[x] for x in f_list]])
#           args.extend([reg_max, end2end, [ch[x] for x in f]])
            if m is Segment or m is YOLOESegment or m is Segment26 or m is YOLOESegment26:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy if detect_legacy is None else bool(detect_legacy)
